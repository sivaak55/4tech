...
var globalEMsg = ''; //to store the global error messagehideModal();
...
$(document).on('pagebeforeshow', 'div[data-role=page]', function (event) {
    ...
    try {
        ...
        if (currentPageId == 'billing-sch-page') {
            var CDate = new Date();
        ...
        if (currentPageId == 'billing-editsch-page') {
            var CDate = new Date();
            CDate = CDate.getMonth() + 1 + "/" + CDate.getDate() + "/" + CDate.getFullYear();
            if (new Date(viewModel.accountSummary.DueDate) >= new Date(CDate)) {
                $('#liduecheck').show();
        ...
        if (currentPageId == 'billing-accounts-edit') {
            $('#econfirmroutingNumber').val(viewModel.selectedAccount().routingNumber());
            $('#econfirmaccountNumber').val(viewModel.selectedAccount().accountNumber());
...
function viewFullSite() {
    ...
    window.location.replace('http://www.bge.com/pages/default.aspx');
}
...
function pageTranslate(selectedPage) {
    ...
    if (!jQuery.isEmptyObject(CurrentAccountSummary)) {
        ...
        // $.mobile.changePage('/'+ globalpath +'/myprograms.html', { transition: "slide" });
            GetPrograms();
        ...
        //$.mobile.changePage('/'+ globalpath +'/sem.html', { transition: "slide" });
            GetOpowerComp();
        else if (selectedPage == 'outages')
        //$.mobile.changePage('/'+ globalpath +'/outages.html', { transition: "slide" });
            GetOutages();
        else if (selectedPage == 'ser')
            showSERHistoricalData();
        //$.mobile.changePage('/'+ globalpath +'/ser.html', { transition: "slide" });
        else if (selectedPage == 'mynotifications')
        //$.mobile.changePage('/'+ globalpath +'/notifications.html', { transition: "slide" });
            getPersonNotifications();
...
$(document).on('pagebeforeshow', '#includehf', function (event) {
    ...
    //$('.ui-footer').load('/'+ globalpath +'/footer.aspx');
...
function GetMessages() {
    ...
    $.ajax({
        type: "POST",
        async: false,
        url: serverPath + "/Bge.Canp/AnonymousService.asmx/GetMessages",
...
function FetchAccountSummary() {
    ...
    $.ajax({
        ...
        url: serverPath + "/_layouts/Bge.Canp/SecureServices.asmx/GetAccountSummary",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        timeout: CanpAjaxTO,
        global: false,
...
function GetOpowerComp() {
    ...
    if (jQuery.isEmptyObject(OpowerData)) {
        ...
        $.ajax({
            type: "POST",
            url: serverPath + "/_layouts/Bge.Canp/SecureServices.asmx/GetOpowerData",
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            async: true,
            timeout: CanpAjaxTO,
...
function OpowerCompError(xhr, ajaxOptions, thrownError) {
    ...
    $('#sem_nodatasimilar').addClass('hidden');
    $('#sem_nodatapast').addClass('hidden');
    $('#uiOpowerErrorTxt').html(findMessage('OPOWER_PARTIAL_ERROR_MOBILE')); // Partial Data Error
...
function OpowerCompComplete(xhr, status) {
    ...
    if (status == 'timeout') {
        ...
        $('#sem_nodatasimilar').addClass('hidden');
        $('#sem_nodatapast').addClass('hidden');
        $('#uiOpowerErrorTxt').html(findMessage('OPOWER_PARTIAL_ERROR_MOBILE'));
    }
    $.mobile.hidePageLoadingMsg();
...
function showBillComparison() {
    ...
    if (!jQuery.isEmptyObject(mErrorMessage)) {
        ...
        $('#billCompareBody').addClass('hidden');
        $('#cpu').removeClass('hidden');
        return;
    }
    ...
    if (!jQuery.isEmptyObject(gasCompData) && !jQuery.isEmptyObject(elecCompData)) {
        ...
        if (!jQuery.isEmptyObject(gasCompData.FailureExplanation) || !jQuery.isEmptyObject(elecCompData.FailureExplanation)) {
            $('#uiOpowerErrorTxtpast').html(findMessage('OPOWER_PARTIAL_ERROR_MOBILE')); // Partial Data Error
            $('#sem_nodatapast').removeClass('hidden');
            $('#billCompareBody').addClass('hidden');
            $('#cpu').removeClass('hidden');
            return;
        ...
        if (!jQuery.isEmptyObject(gasCompData.FailureExplanation)) {
            ...
            $('#sem_comparepast').removeClass('hidden');
            $('#billCompareBody').removeClass('hidden');
            $('#sem_nodatapast').addClass('hidden');
...
function OnChangePremiseAjax() {
    ...
    $.ajax({
        type: "POST",
        async: false,
        url: serverPath + "/_layouts/Bge.Canp/SecureServices.asmx/ChangePremise",
        data: data,
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        timeout: CanpAjaxTO,
...
function GetDisconnect() {
    $.ajax({
        type: "POST",
        url: serverPath + "/_layouts/Bge.Canp/SecureServices.asmx/GetDisconnect",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        timeout: CanpAjaxTO,
        global: false,
        success: function (msg) {
            ...
            if (msg.d.ErrorCode == null && msg.d.ErrorMessage == null) {
                viewModel.ediBilling(msg.d.IsEDIBilling);
                viewModel.summaryBilling(msg.d.IsSummaryBilling);
            }
            viewModel.saveToLocal();


        },
        complete: function () { getAccountAlertsMsgSP(); }
    });
}
...
function ReportAnOutage() {
    ...
    $.ajax({
        type: "POST",
        url: serverPath + "/_layouts/Bge.Canp/SecureServices.asmx/ReportAnOutageData",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        timeout: CanpAjaxTO,
        global: false,
        ...
        complete: RportAnOutageComplete
    });
}
function RportAnOutageComplete(msg) {
    hideModal();
}
...
function GetPrograms() {
    ...
    if ((viewModel.canpPrograms() == null || viewModel.canpPrograms().length == 0) || viewModel.accountSummary.NumberAccounts > 1) {
        ...
        $.ajax({
            type: "POST",
            url: serverPath + "/_layouts/Bge.Canp/SecureServices.asmx/GetPrograms",
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            timeout: CanpAjaxTO,
            async: true,
...
function GetOutages() {
    ...
    if (jQuery.isEmptyObject(OutageData)) {
        ...
        $.ajax({
            type: "POST",
            url: serverPath + "/_layouts/Bge.Canp/SecureServices.asmx/GetOutages",
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            async: true,
            timeout: CanpAjaxTO,
...
function ChangeAccount(accountId, premiseId) {
    ...
    showModal();
    $.ajax({
        url: serverPath + '/_layouts/Bge.Canp/SecureServices.asmx/ChangeAccountPremise',
        type: 'POST',
        async: false,
        headers: {
            "cache-control": "no-cache"
...
function GetAccountsOnPage(search, searchFilter) {
    ...
    showModal();
    $.ajax({
        url: serverPath + '/_layouts/Bge.Canp/SecureServices.asmx/GetAccountsOnPage',
        type: 'POST',
        headers: {
            "cache-control": "no-cache"
        },
...
function getViewBillURL() {
    $.ajax({
        url: serverPath + "/_layouts/Bge.Canp/SecureServices.asmx/GetViewBillURL",
        type: 'POST',
        headers: {
            "cache-control": "no-cache"
        },
...
function getCurrentBillInfo(accountNumber) {
    ...
    showModal();
    $.ajax({
        url: serverPath + '/_layouts/Bge.Canp/SecureServices.asmx/getCurrentBillDetails',
        type: 'POST',
        headers: {
            "cache-control": "no-cache"
        },
...
function showSERHistoricalData() {
    ...
    if (jQuery.isEmptyObject(SERData)) {
        ...
        $.ajax({
            type: "POST",
            async: true,
            url: serverPath + "/_layouts/Bge.Canp/SecureServices.asmx/GetBGESERSavingsSummary",
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            timeout: CanpAjaxTO,
            global: false,
...
function SERSavingsPopulate(msg) {
    ...
    if (jQuery.isEmptyObject(msg.d)) {
        ...
        $('#uiEnergySavingsPnl').addClass('hidden');
        $('.serpageclass').addClass('hidden');
        $('#uiNoDataandError').addClass('hidden');
...
function getPersonNotifications() {
    ...
    if (jQuery.isEmptyObject(NotificationsData)) {
        ...
        $.ajax({
            type: "POST",
            url: serverPath + "/_layouts/Bge.Canp/SecureServices.asmx/GetNotifications",
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            async: true,
            timeout: CanpAjaxTO,
        ...
        $.ajax({
            ...
            success: function (msg) {
                $.each(msg.d.ProgramList, function (idx) {
                    if (msg.d.ProgramList[idx].Name == 'PeakRewards') {
                        isPeakRewards = true;
                    }
...
function fillNotifications() {
    if (!jQuery.isEmptyObject(notifications)) {
        ...
        $.each(notifications.NotificationPreferncesGroups, function (idx) {
            ...
            if (viewModel.accountSummary != null && isPeakRewards && notificationGroups[notifications.NotificationPreferncesGroups[idx]] == 'PeakRewards Events') {
...
function updateNotifications(notificationMedadata) {
    ...
    $.ajax({
        type: "POST",
        url: serverPath + "/_layouts/Bge.Canp/SecureServices.asmx/UpdateNotifications",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
...
function validateCustomerAccount() {
    ...
    $.ajax({
        type: "POST",
        url: serverPath + "/_layouts/Bge.Canp/SecureServices.asmx/ValidateCustomerAccount",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        async: false,
        timeout: CanpAjaxTO,
...
function sessionExpired() {
    ...
    $.ajax({
        type: "POST",
        url: serverPath + "/_layouts/Bge.Canp/SecureServices.asmx/IsValidSession",
        contentType: "application/json; charset=utf-8",
...
function setGreyHeaderOutages() {
    ...
    if (!jQuery.isEmptyObject(CurrentAccountSummary)) {
        ...
        //$(".serviceaddressclass").append('<div><ul class="bullet hbullet"><li><a  data-rel="popup" href="#"  onclick="javascript:OutagesClick();return false;" data-inline="true">Report a Power Outage </a></li></ul><ul class="bullet"><li><a  data-rel="popup" href="http://outagemap.bge.com/m.html" data-inline="true">View Outage Map </a></li></ul></div>');
        if (!jQuery.isEmptyObject(OutageData) && OutageData.d.Status != null && OutageData.d.Status.indexOf('Our records show your power is on. If not, please call 1-877-778-2222 to report a power outage') != -1)
            $(".serviceaddressclass").append('<div><ul class="bullet hbullet"><li><a  data-rel="popup" href="#"  onclick="javascript:OutagesClick();return false;" data-inline="true">Report a Power Outage </a></li><li><a  data-rel="popup" href="http://outagemap.bge.com/m.html" data-inline="true">View Outage Map </a></li></ul></div>');
        else
