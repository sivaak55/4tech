...
var serverPath="https://stg-secure.bge.com";
...
function checkLogin() {
    ...
    if (sessionExpired())
       ...
       // $.mobile.changePage( '/pages/myaccount.aspx', { transition: "slide" });
	   pageTranslate('myaccountp');

    /*viewModel.getFromLocal();
    CurrentAccountSummary = viewModel.accountSummary;
    if (!$.isEmptyObject(CurrentAccountSummary)) {
    $.mobile.changePage('/myaccount.aspx', {
    transition: "slide"
    });
    } else {
    $.mobile.changePage('/signin.aspx', {
    transition: "slide"
    });
    }*/
}
...
$(document).on('pageinit', 'div[data-role=page]', function (event) {
    ...
    (function () {
        var s = document.createElement("script"); s.async = true; s.src = "/MobileApp/js/webtrends.lite.min.js";
    ...
    //validateSession();
    if (sessionExpired()) {
    $.mobile.changePage( '/signin.aspx', { transition: "slide" });
    ...
    try {
        ...
        //$.mobile.changePage( '/pages/signin.aspx', { transition: "slide" });
    }
...
function FedLogin() {
    ...
    showModal();
    $.ajax({
        url: serverPath+'/_layouts/Bge.Canp/SecureServices.asmx/FederatedLogin',
        type: 'POST',
        headers: {
...
function FedLogout() {
    ...
    showModal();
    $.ajax({
        url: serverPath+'/_layouts/Bge.Canp/SecureServices.asmx/Logout',
        type: 'POST',
        headers: {
            "cache-control": "no-cache"
        },
        ...
        complete: function () {
            ...
            /*$.mobile.changePage( '/pages/signin.aspx', {
            transition: "slide"
            });*/
...
function cleanup() {
    ...
    clearTimeout(visa);
    GetMessages();
    /*$.mobile.changePage( '/pages/index.aspx', {
    transition: "slide"
    });*/
    /* } */
...
$(document).on('pageinit', '#fuzzy-account-pao-page', function (event) {
    ...
    $.each(CustomerDataOutage.OutageData, function (idx) {
        var isDisabled = "";
        var outageStatusMsg = "";
        if (this.OutageStatus == "ACTIVE" || this.OutageStatus == "FUZZY" || this.OutageStatus=="CUTOUT") {
            isDisabled = "disabled";
    ...
    $(function () {
        ...
        $('.anchorlist').click(function () {
            ...
            $.mobile.changePage( 'report-outage-form.aspx', {
                transition: "slide"
            });
        });
    });
...
function OutagesClick() {
    if (!$.isEmptyObject(CurrentAccountSummary)) {
        ...
        $.mobile.hidePageLoadingMsg();
        $.mobile.changePage( 'report-an-outage.aspx', {
            transition: "slide"
        })
    }
...
function reportOutage() {
    ...
    showModal();
    $.ajax({
        url: serverPath+'/_layouts/Bge.Canp/AnonymousService.asmx/SubmitOutage',
        type: 'POST',
        headers: {
            "cache-control": "no-cache"
        },
        ...
        success: function (data, textStatus, xhr) {
            if (data.d.ReturnStatus == "ERROR") {
                ...
                // $.mobile.changePage('/'+ globalpath +'/pages/report-outage-success.aspx', { transition: "slide" });
            }
        ...
        complete: function () {
            ...
            $.mobile.changePage( 'report-outage-success.aspx', {
                transition: "slide"
            });
            hideModal();
        }
...
/*gets the call from report-outage-success.aspx*/
...
/*gets the call from outage-form.aspx*/
    ...
    if (!$.isEmptyObject(CurrentAccountSummary)) {
        ...
        if (CurrentAccountSummary.ServiceList != null && CurrentAccountSummary.ServiceList.length > 1) {
            ...
            $.each(CustomerDataOutage.OutageData, function (idx) {
                ...
                pselect.append('<option  id=add-' + idx + ' value="' + this.FullAddress + '" etrdes="' + this.ETRDescription + '" >' + this.FullAddress + '</option>');
                addressFound++;
            });
...
$(document).on('pagebeforeshow', '#outage-summary-pao-page', function (event) {
    ...
    if (CustomerDataOutage) {
        ...
        else if (CustomerDataOutage.OutageData[0].OutageStatus!="" && CustomerDataOutage.OutageData[0].OutageStatus=="CUTOUT"){
            	$('#outage_etr').text(findMessage('RAO_FAILED_CUTOUT'));//get the CUTOUT error message from Messages.xml
        }else{
...
$(document).on('pageinit', '#report-outage-page', function (event) {
    // checking if the session is exists or not 
    //OutagesClick();
...
function getRAOAddress() {
    ...
    showModal();
    $.ajax({
        url: serverPath+'/_layouts/Bge.Canp/AnonymousService.asmx/GetCustomerDataOutage',
        type: 'POST',
        headers: {
            "cache-control": "no-cache"
        },
            ...
            if (data.d.ReturnStatus == "ERROR" && data.d.ErrorCode == "ACCOUNT NOT FOUND") {
                ...
                if (data.d.ErrorCode == "InvalidCaptcha") {
                    ...
                    if (data.d.ErrorCode == "ACCOUNT NOT FOUND") {
                        ...
                        $('#account-error').show();
                        hideModal();
                        //redirect to report-outage-form.aspx page to show the error message
                        if (!$.isEmptyObject(CurrentAccountSummary)) {
                ...
                if (CustomerDataOutage.OutageData.length > 1) {
                    if ($.isEmptyObject(CurrentAccountSummary)) {
                        $.mobile.hidePageLoadingMsg();
                        $(".bgepanel").panel("close");
                        $.mobile.changePage( 'outage-fuzzy-account.aspx', {
                            transition: "slide"
                        });
                    } else {
                        $.mobile.hidePageLoadingMsg();
                    ...
                    if (CustomerDataOutage.OutageData[0].OutageStatus == "ACTIVE" || CustomerDataOutage.OutageData[0].OutageStatus == "FUZZY") {
                        $.mobile.hidePageLoadingMsg();
                        $(".bgepanel").panel("close");
                        $.mobile.changePage( 'report-outage-summary.aspx', {
                            transition: "slide"
                        });
                    } else {
                        $.mobile.hidePageLoadingMsg();
...
function getAlertsMsgSP() {
    ...
    showModal();
    $.ajax({
        url: serverPath+'/_layouts/Bge.Canp/AnonymousService.asmx/GetGlobalAlertMessage',
        type: 'POST',
        headers: {
            "cache-control": "no-cache"
        },
...
function getAccountAlertsMsgSP() {
    ...
    $.ajax({
        url: serverPath+'/_layouts/Bge.Canp/SecureServices.asmx/GetAlertMessage',
        type: 'POST',
        headers: {
            "cache-control": "no-cache"
        },
...
function ContactClick() {
    ...
    if (!$.isEmptyObject(CurrentAccountSummary)) {
        ...
        $.mobile.hidePageLoadingMsg();
        $(".bgepanel").panel("close");
        $.mobile.changePage( 'contactus.aspx', {
            transition: "slide"
        })
    }
...
$(document).on('pageinit', '#contact-us-page', function (event) {
    ...
    $('#general_entry').click(function () {
        recognized = false;
        $.mobile.changePage( 'contact-form.aspx', {
            transition: "slide"
        });
    });
    ...
    var validator = $form.validate({
        ...
        submitHandler: function (form) {
            ...
            if (FormTypeID != null && FormTypeID != "") {
                ...
                $.mobile.changePage( 'contact-success.aspx', {
                    transition: "slide"
                });
...
$(document).on('pageinit', '#fuzzy-account-con-page', function (event) {
    ...
    $.each(cleanedDisplayAddress, function (idx) {
    	//if(cleanedDisplayAddress[recordsFound] != '')
    	//{
...
function getAddress() {
    ...
    showModal();
    $.ajax({
        //url: serverPath+'/_layouts/Bge.Canp/AnonymousService.asmx/GetCustomerDetails',
        url: serverPath+'/_layouts/Bge.Mobile.Services/AnonymousService.asmx/GetCustomerDetails',
        type: 'POST',
        headers: {
            "cache-control": "no-cache"
        },
        ...
        success: function (data, textStatus, xhr) {
            ...
            if (data.d.ReturnStatus == "ERROR") {
                ...
                if (checkNull(cleanedDisplayAddress).length > 1) {
                    ...
                    $.mobile.hidePageLoadingMsg();
                    $(".bgepanel").panel("close");
                    $.mobile.changePage( 'contact-fuzzy-account.aspx', {
                        transition: "slide"
                    });
...
function submitContactUsForm(formMetadata) {
    ...
    showModal();
    $.ajax({
        url: serverPath+'/_layouts/Bge.Mobile.Services/AnonymousService.asmx/SubmitContactUsForm',
        type: 'POST',
        headers: {
            "cache-control": "no-cache"
        },
...
function getUserProfile() {
    ...
    $.ajax({
        type: "POST",
        url: serverPath+"/_layouts/Bge.Canp/SecureServices.asmx/GetUserProfileData",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        async: false,
        timeout: CanpAjaxTO,
        ...
        success: function (data, textStatus, xhr) {
            ...
            $.mobile.hidePageLoadingMsg();
            $.mobile.changePage( 'myprofile.aspx', {
                transition: "slide"
            });
        },
...
function UpdateProfileDetails() {
    ...
    if (ko.mapping.fromJSON(localStorage.CustomerData).contactDetails() == ko.mapping.toJSON(viewModel.contactDetails()) || !SE_regex.test($('#secondaryEmail').val()) || !E_regex.test($('#primaryEmail').val()) || jQuery.isEmptyObject($('#primaryEmail').val()) || !p_regex.test(viewModel.contactDetails().PhoneNumber()[0].CompleteNumber()) || !p_regex.test(viewModel.contactDetails().PhoneNumber()[1].CompleteNumber()) || !p_regex.test(viewModel.contactDetails().PhoneNumber()[2].CompleteNumber())) {
        ...
        submitData.contactDetails.PhoneNumber[0].CompleteNumber = submitData.contactDetails.PhoneNumber[0].CompleteNumber.replace(/[^a-z0-9\s]/gi,'').replace(/\s+/, "");
        submitData.contactDetails.PhoneNumber[1].CompleteNumber = submitData.contactDetails.PhoneNumber[1].CompleteNumber.replace(/[^a-z0-9\s]/gi,'').replace(/\s+/, "");
        submitData.contactDetails.PhoneNumber[2].CompleteNumber = submitData.contactDetails.PhoneNumber[2].CompleteNumber.replace(/[^a-z0-9\s]/gi,'').replace(/\s+/, "");
        $.mobile.showPageLoadingMsg();
        showModal();
        $.ajax({
            type: "POST",
            ...
            url: serverPath+"/_layouts/Bge.Canp/SecureServices.asmx/UpdateUserProfileData",
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            success: function (data, textStatus, xhr) {
                if (data.d.ErrorCode == null && data.d.ErrorMessage == null) {
                    ...
                    $.mobile.changePage( 'myprofileconfirm.aspx', {
                        transition: "slide"
                    });
                }
...
function CANPViewModel() {
    ...
    self.ediBilling = ko.observable(false);
    self.summaryBilling = ko.observable(false);
    ...
    self.getFromLocal = function () {
        if (localStorage.length > 0) {
            ...
            self.ediBilling(ko.mapping.fromJSON(localData.ediBilling)());
            self.summaryBilling(ko.mapping.fromJSON(localData.summaryBilling)());
    ...
    self.saveToLocal = function () {
        ...
        localData.ediBilling = ko.mapping.toJSON(self.ediBilling);
        localData.summaryBilling = ko.mapping.toJSON(self.summaryBilling);
    ...
    self.initAccounts = function () {
        ...
        $.ajax({
            ...
            url: serverPath+"/_layouts/Bge.Canp/SecureServices.asmx/GetPaymentAccounts",
            contentType: "application/json; charset=utf-8",
            dataType: "json",
    ...
    self.addAccount = function () {
        ...
        viewModel.saveToLocal();
        $.mobile.changePage( 'pmt-accounts-new.aspx', {
            transition: "slide"
        });
    }
    ...
    self.addPayment = function () {
        ...
        if (viewModel.bankAccounts().length != 0 && viewModel.findStatus('active').length > 0) {
            ...
            viewModel.saveToLocal();
            $.mobile.changePage( 'pmt-schedule.aspx', {
                transition: "slide"
            });
        } else {
            this.addAccount();
    ...
    self.delrecPayment = function () {
        $.mobile.changePage( 'pmt-automated-dconfirm.aspx', {
            transition: "slide"
        });
    ...
    self.editrecPayment = function () {
        viewModel.saveToLocal();
        $.mobile.changePage( 'pmt-edit-automated.aspx', {
            transition: "slide"
        });
    }
    ...
    self.editschedPayment = function () {
        ...
        viewModel.saveToLocal();
        $.mobile.changePage( 'pmt-edit-schedule.aspx', {
            transition: "slide"
        });
    }
    ...
    self.editconfirmrecPayment = function () {
        ...
        viewModel.saveToLocal();
        $.mobile.changePage( 'pmt-automated-econfirm.aspx', {
            transition: "slide"
        });
    }
    ...
    self.recPayment = function () {
        ...
        viewModel.saveToLocal();
        $.mobile.changePage( 'pmt-automated.aspx', {
            transition: "slide"
        });
    ...
    self.viewAccount = function (account) {
        ...
        viewModel.saveToLocal();
        $.mobile.changePage( 'pmt-accounts-view.aspx', {
            transition: "slide"
        });
    }

    self.editAccount = function () {
        $.mobile.changePage( 'pmt-accounts-edit.aspx', {
            transition: "slide"
        });
    }
    ...
    self.resetAccount = function () {
        ...
        $.mobile.changePage( 'pmt-accounts.aspx', {
            transition: "slide"
        });
    }
    ...
    self.createAccount = function () {
        ...
        if ($.isEmptyObject($('#nickName').val()) || $.isEmptyObject($('#accountHolderName').val()) || $.isEmptyObject($('#routingNumber').val()) || $.isEmptyObject($('#accountNumber').val()) || $.isEmptyObject($('#confirmroutingNumber').val()) || $.isEmptyObject($('#confirmaccountNumber').val())) {
            $('#addAccounts-Error').show();
            $('#addAccounts-Error').empty();
            $('#addAccounts-Error').append('All fields are required.');
            return;
        }
        else if (!validateRouting($('#routingNumber').val()) || !validateRouting($('#confirmroutingNumber').val()) || !Bankaccount_regex.test($('#accountNumber').val()) || !Bankaccount_regex.test($('#confirmaccountNumber').val())) {
            ...
            if (!validateRouting($('#confirmroutingNumber').val())) {
                $('#addAccounts-Error').show();
                $('#addAccounts-Error').empty();
                $('#addAccounts-Error').append('Confirm Routing Number is not valid.');
                return;
            }
            if (!Bankaccount_regex.test($('#confirmaccountNumber').val())) {
                $('#addbankAccount-Error').show();
                $('#addbankAccount-Error').empty();
                $('#addbankAccount-Error').append('Confirm Account Number is not valid.');
                return;
            }
        ...
        if ($('#accountNumber').val() != $('#confirmaccountNumber').val()) {
            $('#addbankAccount-Error').show();
            $('#addbankAccount-Error').empty();
            $('#addbankAccount-Error').append('Confirm Account Number does not match.');
            return;
        }
        if ($('#routingNumber').val() != $('#confirmroutingNumber').val()) {
            $('#addbankAccount-Error').show();
            $('#addbankAccount-Error').empty();
            $('#addbankAccount-Error').append('Confirm Routing Number does not match.');
            return;
        }
        viewModel.saveToLocal();
        $.mobile.changePage( 'pmt-accounts-nconfirm.aspx', { transition: "slide" });
    ...
    self.confirmCreateAccount = function () {
        ...
        $.ajax({
            ...
            url: serverPath+"/_layouts/Bge.Canp/SecureServices.asmx/CreatePaymentAccount",
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            success: function (allData) {
                if (allData.d.ErrorCode == null && allData.d.ErrorMessage == null) {
                ...
                $.mobile.changePage( 'pmt-accounts-nfinal.aspx', {
                    transition: "slide"
                });
            },
            async: true,
    ...
    self.updateAccount = function () {
        ...
        if ($.isEmptyObject($('#nickName').val()) || $.isEmptyObject($('#accountHolderName').val()) || $.isEmptyObject($('#routingNumber').val()) || $.isEmptyObject($('#accountNumber').val()) || $.isEmptyObject($('#econfirmroutingNumber').val()) || $.isEmptyObject($('#econfirmaccountNumber').val())) {
            $('#editAccounts-Error').show();
            $('#editAccounts-Error').empty();
            $('#editAccounts-Error').append('All fields are required.');
            return;
        } else if (!validateRouting($('#routingNumber').val()) || !Bankaccount_regex.test($('#accountNumber').val()) || !validateRouting($('#econfirmroutingNumber').val()) || !Bankaccount_regex.test($('#econfirmaccountNumber').val())) {
            if (!validateRouting($('#routingNumber').val())) {
                $('#editAccounts-Error').show();
                $('#editAccounts-Error').empty();
            ...
            if (!validateRouting($('#econfirmroutingNumber').val())) {
                $('#editAccounts-Error').show();
                $('#editAccounts-Error').empty();
                $('#editAccounts-Error').append('Confirm Routing Number is not valid.');
                return;
            }
            if (!Bankaccount_regex.test($('#econfirmaccountNumber').val())) {
                $('#editbankAccount-Error').show();
                $('#editbankAccount-Error').empty();
                $('#editbankAccount-Error').append('Confirm Account Number is not valid.');
                return;
            }
        }
        if ($('#accountNumber').val() != $('#econfirmaccountNumber').val()) {
            $('#editbankAccount-Error').show();
            $('#editbankAccount-Error').empty();
            $('#editbankAccount-Error').append('Confirm Account Number does not match.');
            return;
        }
        if ($('#routingNumber').val() != $('#econfirmroutingNumber').val()) {
            $('#editbankAccount-Error').show();
            $('#editbankAccount-Error').empty();
            $('#editbankAccount-Error').append('Confirm Routing Number does not match.');
            return;
        }

        viewModel.saveToLocal();
        $.mobile.changePage( 'pmt-accounts-econfirm.aspx', { transition: "slide" });
    }
    ...
    self.confirmUpdateAccount = function () {
        ...
        $.ajax({
            ...
            url: serverPath+"/_layouts/Bge.Canp/SecureServices.asmx/EditPaymentAccount",
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            success: function (allData) {
                if (allData.d.ErrorCode == null && allData.d.ErrorMessage == null) {
                ...
                $.mobile.changePage( 'pmt-accounts-efinal.aspx', {
                    transition: "slide"
                });
            },
            async: true,
    ...
    self.deleteAccount = function () {
        viewModel.saveToLocal();
        $.mobile.changePage( 'pmt-accounts-dconfirm.aspx', { transition: "slide" });
    ...
    self.ConfirmDeleteAccount = function () {
        ...
        $.ajax({
            ...
            url: serverPath+"/_layouts/Bge.Canp/SecureServices.asmx/DeletePaymentAccount",
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            success: function (allData) {
                if (allData.d.ErrorCode == null && allData.d.ErrorMessage == null) {
                ...
                $.mobile.changePage( 'pmt-accounts-dfinal.aspx', {
                    transition: "slide"
                });
            },
            async: true,
    ...
    self.getpaymentDetails = function (encryptedTransactionId) {
        ...
        $.ajax({
            ...
            url: serverPath+"/_layouts/Bge.Canp/SecureServices.asmx/QueryPaymentDetails",
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            timeout: CanpAjaxTO,
            global: false,
            success: function (msg) {
                ...
                $.mobile.changePage( 'pmt-transaction.aspx', {
                    transition: "slide"
                });
    ...
    self.confirmschedulePayment = function () {
        $.mobile.changePage( 'pmt-schedule-cconfirm.aspx', {
            transition: "slide"
        });
    ...
    self.cancelschedulePayment = function (encryptedTransactionId) {
        ...
        $.ajax({
            ...
            data: JSON.stringify(paymentTransID),
            url: serverPath+"/_layouts/Bge.Canp/SecureServices.asmx/CancelPayment",
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            timeout: CanpAjaxTO,
            global: false,
            success: function (msg) {
                ...
                $.mobile.changePage( 'pmt-schedule-dfinal.aspx', {
                    transition: "slide"
                });
                // hideModal();
            },
    ...
    self.queryPayments = function () {
        ...
        $.ajax({
            ...
            url: serverPath+"/_layouts/Bge.Canp/SecureServices.asmx/QueryAllPayments",
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            success: function (allData) {
                if (allData.d.ErrorCode == null && allData.d.ErrorMessage == null) {
                    ...
                    $.mobile.changePage( 'pmt-activity.aspx', {
                        transition: "slide"
                    });
                } else {
    ...
    self.submitEditPayment = function () {
        ...
        $.mobile.changePage( 'pmt-schedule-econfirm.aspx', {
            transition: "slide"
        });
    }
    ...
    self.submitPayment = function () {
        ...
        $.mobile.changePage( 'pmt-schedule-confirm.aspx', {
            transition: "slide"
        });
    }
    ...
    self.editschedulePayment = function () {
        ...
        showModal();
        $.ajax({
            url: serverPath+'/_layouts/Bge.Canp/SecureServices.asmx/EditPayment',
            type: 'POST',
            data: ko.mapping.toJSON(submitData),
            headers: {
                "cache-control": "no-cache"
            ...
            success: function (allData) {
                ...
                if (allData.d.ErrorCode == null && allData.d.ErrorMessage == null) {
                    ...
                    $.mobile.changePage( 'pmt-schedule-efinal.aspx', {
                        transition: "slide"
                    });
                } else {
    ...
    self.schedulePayment = function () {
        ...
        showModal();
        $.ajax({
            url: serverPath+'/_layouts/Bge.Canp/SecureServices.asmx/SchedulePayment',
            type: 'POST',
            data: ko.mapping.toJSON(submitData),
            headers: {
                "cache-control": "no-cache"
            ...
            success: function (allData) {
                ...
                $.mobile.changePage( 'pmt-schedule-final.aspx', {
                    transition: "slide"
                });
                //hideModal();
            },
    ...
    self.queryrecurringPayment = function () {
        ...
        $.ajax({
            ...
            url: serverPath+"/_layouts/Bge.Canp/SecureServices.asmx/QueryRecurringPayment",
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            success: function (allData) {
                if (allData.d.ErrorCode == null && allData.d.ErrorMessage == null) {
                    ...
                    viewModel.saveToLocal();
                    statusModel.queryPaymentDetailsError(false);
                    $.mobile.changePage( 'pmt-view-recurring.aspx', {
                        transition: "slide"
                    });
    ...
    self.submitrecurringPayment = function () {
        ...
        viewModel.saveToLocal();
        $.mobile.changePage( 'pmt-automated-confirm.aspx', {
            transition: "slide"
        });
    ...
    self.addrecurringPayment = function () {
        ...
        $.ajax({
            ...
            data: ko.mapping.toJSON(submitData),
            url: serverPath+"/_layouts/Bge.Canp/SecureServices.asmx/AddRecurringPayment",
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            timeout: CanpAjaxTO,
            global: false,
            success: function (allData) {
                ...
                $.mobile.changePage( 'pmt-automated-final.aspx', {
                    transition: "slide"
                });
            },
            complete: function (allData) {
    ...
    self.editrecurringPayment = function () {
        ...
        $.ajax({
            ...
            data: ko.mapping.toJSON(submitData),
            url: serverPath+"/_layouts/Bge.Canp/SecureServices.asmx/EditRecurringPayment",
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            timeout: CanpAjaxTO,
            global: false,
            success: function (allData) {
                ...
                $.mobile.changePage( 'pmt-automated-efinal.aspx', {
                    transition: "slide"
                });
            },
            complete: function (allData) {
    ...
    self.deleterecurringPayment = function () {
        ...
        $.ajax({
            ...
            url: serverPath+"/_layouts/Bge.Canp/SecureServices.asmx/DeleteRecurringPayment",
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            success: function (allData) {
                if (allData.d.ErrorCode == null && allData.d.ErrorMessage == null) {
                ...
                $.mobile.changePage( 'pmt-automated-dfinal.aspx', {
                    transition: "slide"
                });
            },
            complete: function (allData) {
